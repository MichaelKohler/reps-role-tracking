...
class Reps {
  ...
  static getWithMentorInfo() {
    ...
    // TODO: optimize this in terms of memory usage, but for now, we have it ;)
    const allReports = storageHandler.getStorageItem('repsactivities');
    return this.getAll()
      .then((reps) => {
        reps.forEach((rep) => {
          rep.isMentor = false;
          ...
          // Get last report, seriously, memory and time!!! :()
          const repsDisplayName = rep.display_name;
          const allReportsByRep = _.filter(allReports, { activityDetails: { user: { display_name: repsDisplayName } } });
          const sortedReports = allReportsByRep.sort((a, b) => {
            const aReportDate = new Date(a.activityDetails.report_date);
            const bReportDate = new Date(b.activityDetails.report_date);
            if (aReportDate > bReportDate) {
              return -1;
            } else if (bReportDate > aReportDate) {
              return 1;
            } else {
              return 0;
            }
          ...
          if (lastReport) {
            rep.lastReportDate = new Date(lastReport.activityDetails.report_date);
          }
        });
