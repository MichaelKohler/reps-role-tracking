...
class RemoHelper {
  ...
  /**
   * Initializes a query for all Reps activities until a certain
   * date back in the past
   *
   * @param  {Number} numberOfPages  number of pages to fetch
   *
   * @return {Promise}    Promise which resolves with all activities found
   */
  getAllActivities(numberOfPages) {
    this.url = 'https://reps.mozilla.org/api/remo/v1/activities/?format=json';

    return new Promise((resolve, reject) => {
      this.fetchPagesRecursively(this.url, resolve, reject, numberOfPages);
    });
  }
